...
`features-nr` y `features-plus` envuelven `startQuiz` / `makeQuizItems` / `renderQuiz` / `applySpeakVerdict`. El orden de bootstrap importa (NR luego PLUS). Prefiere un `if (quiz.mode === "…")` nativo en `app.js` antes que otra envoltura.
